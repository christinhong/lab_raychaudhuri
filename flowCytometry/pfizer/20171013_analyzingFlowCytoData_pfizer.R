...
wd <- "~/00_SR-lab-data/pfizer/data/FACS/B0151006-raw-fcs/"
setwd(wd)
...
# These are recommendations in file:///Users/chong/00_SR-lab-data/pfizer/data/FACS/B0151006-raw-fcs/data_survey.html
annot <- read.delim("fcs_annotation.txt", check.names=FALSE)

it <- which(annot$anonid == "D00000000184161" & annot$visit=="Week 0 / Day 1") # This is to get row numbers of filter matches.  Checked with "annot[51:55, ]"

is <- order(annot[it,"TUBE NAME"]) # Orders matches by tube name

kable(annot[it[is], c("anonid", "visit", "TUBE NAME", "FCSFILE_PATH")], row.names=FALSE) # Shows ordered matches (kable just puts it in a pretty table format)

stopifnot(all(file.exists(as.character(annot[it,"FCSFILE_PATH"])))) # Checks that all files linked in "it" exist

# Hmm...some interesting tricks here, but a bit limited.  Still, I can use this structure.  Basically keeps annotations in file vs. FCS filename.  That said...I think I need to maintain this structure in order to properly annotate which file is what, or spend a LOT of time manually editing everything.  Hmmmmm.  I guess that means figuring out flowCore after all.
