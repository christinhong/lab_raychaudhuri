...
# So much easier!  (Though to be fair, that's mainly because I can now reuse code for ggplot, whereas I had to go find and learn everything anew for mosaicplot.)

### TRY NEXT TIME WITH "position_fill" option in ggplot - may be able to make mosaic too.
...
# Save results in PDF
pdf("2015-09-30_aa_RNA-02-DE_NUM_replottingData.pdf",width = 8, height = 8, useDingbats = F);


# Re-plotting with filtered reads

head(L2fcTable)

L2fcTable2 <- L2fcTable[,-c(2)] # Remove Length column
L2fcTable3 <- L2fcTable2[,-c(1)] # Remove GeneID column
rownames(L2fcTable3) <- L2fcTable[,1]
head(L2fcTable2)
head(L2fcTable3)

# Only keep in the analysis those genes which had >10 reads per million mapped reads in at least two libraries.
keep = rowSums(cpm(L2fcTable3) > 10) >= 2
L2Count.filt = L2fcTable2[keep, ] # Still has GeneID
L2Count2.filt = L2fcTable3[keep, ] # Doesn't have GeneID

dim(L2fcTable2) # 57820 rows, 9 columns
dim(L2Count.filt) # 40057 rows if filtering rowSum > 0, 26993 if filtering for genes with >10 reads per million in at least 2 libraries
dim(L2Count2.filt)
head(L2Count.filt)
head(L2Count2.filt) 
...
bx = melt(L2Count.filt, id=c("GeneID"),
          measure.vars = c("HC1",
                           "HC2",
                           "HC3",
                           "HC4",
...
ggplot(L2Count.filt, aes(x = HC1)) + ylab(expression(log[2](count+1))) + geom_histogram(binwidth = 0.3, color = "#FFFFFF") + ggtitle("Psuedocounts for HC1 after filtering \n (>10 cpm in at least 2 libraries)")


ggplot(bx2, aes(x = Samples, y = value, fill = Condition)) + geom_boxplot() + xlab("") + ylab(expression(log[2](count + 1))) + scale_fill_manual(values = c("#999999", "#00ccff")) + ggtitle("Count distribution of null treatment samples \n (>10 cpm in at least 2 libraries)")


ggplot(bx2, aes(x = value, color = Samples, fill = Samples)) + ylim(c(0,0.25)) + geom_density(alpha = 0.2, size = 1.25) + theme(legend.position = "top") + xlab(expression(log[2](count + 1))) + facet_wrap(~ Condition) + ggtitle("Density plots of null treatment samples \n (>10 cpm in at least 2 libraries)")



L2Count2.filt.ma <- data.matrix(L2Count2.filt, rownames.force = NA) # Must store data.matrix
str(L2Count2.filt.ma[, 1:4]) # Checking if conversion worked
dim(L2Count2.filt.ma)
head(L2Count2.filt.ma)
...
# Save results in PDF
pdf("2015-09-30_aa_RNA-02-DE_NUM_replottingDataClusteredHeatmap.pdf",width = 6, height = 6, useDingbats = F);

# Heatmap {mixOmics}
mat.dist <- L2Count2.filt.ma
...
cim(mat.dist2, color = rev(hmcol), symkey = F, margins = c(5,5), transpose = F, main = "Null treatment sample clustering\n (>10 cpm in at least 2 libraries)")
# Note: "By  default  the  used  clustering  method  for  rows  and  columns  is  the complete  linkage method and the used distance measure is the distance euclidean."


dev.off();

# Save results in PDF
pdf("2015-09-30_aa_RNA-02-DE_NUM_replottingDataPCA.pdf",width = 8, height = 8, useDingbats = F);


# PCA
rv = rowVars(L2Count2.filt.ma)
select = order(rv, decreasing = T)[1:1350] # 2nd number here = how many of the most variable genes should be used for calculating PCA
pca <- prcomp(t(L2Count2.filt.ma[select, ]), scale = T)

sink(file="2015-09-30_PCASummary.txt")
summary(pca)
sink()

plot(pca, type="lines", main="PCA of null samples from 5% most variable genes \n(1350/26993 total genes after filtering for >10 cpm in at least 2 libraries, \nPCA determined by prcomp)", ylim=c(0, 500))
title(xlab="Principle Component")


# From sdev calculated by PCA, the variances are the squared values.
names(pca)
vPcaProp <- pca$sdev^2 / sum(pca$sdev^2)
vPcaPropPercent <- (pca$sdev^2 / sum(pca$sdev^2))*100

vPcaPropCumSumPercent <- cumsum(vPcaPropPercent)

plot(vPcaPropPercent, type="both", main="PCA of null samples from 5% most variable genes \n(1350/26993 total genes after filtering for >10 cpm in at least 2 libraries, \nPCA determined by prcomp)", ylim=c(0, 50), ylab='Proportion of variance (%)', xlab = 'Principle Component', xaxp  = c(1, 9, 8))

plot(vPcaPropCumSumPercent, type="both", main="PCA of null samples from 5% most variable genes \n(1350/26993 total genes after filtering for >10 cpm in at least 2 libraries, \nPCA determined by prcomp)", ylim=c(0, 100), ylab='Cumulative proportion of variance (%)', xlab = 'Principle Component', xaxp  = c(1, 9, 8))
...
#annot = AnnotatedDataFrame(data = data.frame(group, row.names = colnames(L2Count2.filt.ma)))
#expSet = new("ExpressionSet", exprs = as.matrix(L2Count2.filt.ma), phenoData = annot)
...
scores <- data.frame(group, pca$x[,1:3])

# qplot(x=PC1, y=PC2, data=scores) + theme(legend.position="right") + geom_point(colour="#000000", size = 5) + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#00ccff")) # Has outline around dots, but that's not necessary


# Red/salmon dots
qplot(x=PC1, y=PC2, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991")) + ggtitle("Null treatment samples, PC 1 and 2 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)") + coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, wise = NULL)

qplot(x=PC1, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991")) + ggtitle("Null treatment samples, PC 1 and 3 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)") + coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, wise = NULL)

qplot(x=PC2, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991")) + ggtitle("Null treatment samples, PC 2 and 3 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)") + coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, wise = NULL)


# Blue dots IMPROPERLY SCALED
qplot(x=PC1, y=PC2, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#00ccff")) + ggtitle("Null treatment samples, PC 1 and 2 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)")

qplot(x=PC1, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#00ccff")) + ggtitle("Null treatment samples, PC 1 and 3 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)")

qplot(x=PC2, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#00ccff")) + ggtitle("Null treatment samples, PC 2 and 3 \n (>10 cpm in at least 2 libraries, PCA determined by prcomp)")
...
#pc1.2 <- qplot(x=PC1, y=PC2, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991"))

#pc1.3 <- qplot(x=PC1, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991"))

#pc2.3 <- qplot(x=PC2, y=PC3, data=scores) + theme(legend.position="right") + geom_point(aes(colour = factor(group)), size = 4) + scale_colour_manual(values = c("#333333", "#FF6991"))

#grid.arrange(pc1.3, pc1.3, pc2.3, ncol = 3)
...
pcaGroup.df <- data.frame(pcaWithGroups)
str(pcaGroup.df[, 1:4])

## Add column with groups
pcaGroup.df$color <- c("#333333","#333333","#333333","#333333", "#00ccff", "#00ccff", "#00ccff", "#00ccff", "#00ccff")

head(pcaGroup.df)

plot3d(pcaGroup.df[,1:3], col=pcaGroup.df$color, size=2, box = T, type = "s")
aspect3d("iso")
texts3d(pcaGroup.df[,1:3], texts = rownames(pcaGroup.df), adj = 1) # Having issues with FreeType.  Oh well.
snapshot3d("2015-09-30_3dPCA1.png", fmt="png", top=T)
snapshot3d("2015-09-30_3dPCA2.png", fmt="png", top=T)
snapshot3d("2015-09-30_3dPCA3.png", fmt="png", top=T)
snapshot3d("2015-09-30_3dPCA4.png", fmt="png", top=T)

plot3d(pcaGroup.df[,4:6], col=pcaGroup.df$color, size=2, box = T, type = "s")
aspect3d("iso")
texts3d(pcaGroup.df[,4:6], texts = rownames(pcaGroup.df), adj = 1) # Having issues with FreeType.  Oh well.
snapshot3d("2015-09-30_3dPCA5.png", fmt="png", top=T)
snapshot3d("2015-09-30_3dPCA6.png", fmt="png", top=T)
