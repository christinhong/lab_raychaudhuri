...
############################################################################
####		DOWNLOADED FROM MONTGOMERY LAB WEBPAGE ON DEC 5, 2014 		####
#### http://montgomerylab.stanford.edu/resources/kukurba2013/index.html ####
############################################################################
## MODIFIED PATH TO GENCODE FILE (LINE 929)
## MODIFIED PATH TO R (LINE 30)
## MODIFIED SUBROUTINE _load_site_filter(), SO THAT CHRS ARE STORED ONLY WITH NUMBERS (AND NOT CHARACTERS)

## MARCH 11, 2015: MODIFIED SNP2GENE MAPPING TO INCLUDE NON PROTEIN-CODING GENES, INTRONS AND REPORT INTERGENIC
...
sub getGencodeRefGenes() {
    ...
    my $gencode_data_file = "/data/srlab/cmhong/ref-hg19/gencode-v19-chr.gtf";
    ...
    while (my $line = <GENCODE>) {
        ...
        if ($ts[2] eq "exon") {
            ...
            if (!defined($gene_type)) {
            ...
            } else{ #old: elsif ($gene_type eq "protein_coding") {
                ...
                my %exon_object = ('s' => $ts[3],
                                  'e' => $ts[4],
                                  #new: added gene type to end of ID below
                                  'i' => $gene_id . ":" . $transcript_id . ":exon_" . $ts[3] . "_" . $ts[4] . ":" . $gene_name . "-" . $gene_type);
                my $chr = $ts[0];
                $chr =~ s/chr//;
...
#########new: adding case for "gene" instead of "exon" (for including SNPs in introns)
		elsif ($ts[2] eq "gene") {
            my $gene_type;
            my $gene_id;
            my $gene_name;
            ...
            if (!defined($gene_type)) {
                ...
                my $start_index = int($ts[3] / $index);
                my $end_index = int($ts[4] / $index);
                my %gene_object = ('s' => $ts[3],
                                  'e' => $ts[4],
                                  #added gene type to end of ID below
                                  'i' => $gene_id . ":" . $transcript_id . ":exon_" . $ts[3] . "_" . $ts[4] . ":" . $gene_name . "-" . $gene_type);
                my $chr = $ts[0];
                $chr =~ s/chr//;
                for(my $i = $start_index; $i <= $end_index; $i++) {
                    push @{$gencode_gene_lookup{$chr}{$i}}, \%gene_object;
                }
            }
        }       
    }
    ...
    while (my $line = <INCLUDED_SITES>) {
        ...
        my $rsid = $ts[2];
        my $printed = 0;
        my $notInExon = 0;#new: added flag in case SNP is not found in exon

        my @overlapping_exons;
        my @overlapping_genes;#new: added array for gene case
        my $loc_index = int($loc / $index);
        if (!defined($gencode_exon_lookup{$chr}{$loc_index})) {
        	$notInExon = 1; #new: added flag to look in genes if SNP not in exon
            #old: next;
        }else{#added else here because removed next before
        	...
        	foreach my $exon_to_search_for_overlap (@exons_to_search_for_overlap) {
        	    if ($exon_to_search_for_overlap->{'s'} <= $loc && $exon_to_search_for_overlap->{'e'} >= $loc) {
        	        if (!$printed) {
        	            print $chr . "\t" . $loc . "\t" . $rsid . "\t";
        	            print $exon_to_search_for_overlap->{'i'} . "-exon";#new: added "-exon" at the end of id
        	            $printed = 1;
        	        } else {
        	            print ";" . $exon_to_search_for_overlap->{'i'} . "-exon";#new: added "-exon" at the end of id
        	        }
...
######new: adding case for no overlap with exon, to check in genes (hence, intronic)
    	if($printed == 0 or $notInExon == 1){
    	    if (!defined($gencode_gene_lookup{$chr}{$loc_index})) {#not in intron either
        		print $chr . "\t" . $loc . "\t" . $rsid . "\tintergenic\n";#printing SNP even if intergenic
        		next;       	    
        	}else{#added else here because removed next before
        		my @genes_to_search_for_overlap = @{$gencode_gene_lookup{$chr}{$loc_index}};
        		foreach my $gene_to_search_for_overlap (@genes_to_search_for_overlap) {
        		    if ($gene_to_search_for_overlap->{'s'} <= $loc && $gene_to_search_for_overlap->{'e'} >= $loc) {
        		        if (!$printed) {
        		            print $chr . "\t" . $loc . "\t" . $rsid . "\t";
        		            print $gene_to_search_for_overlap->{'i'} . "-intron";#added "-intron" at the end of id
        		            $printed = 1;
        		        } else {
        		            print ";" . $gene_to_search_for_overlap->{'i'} . "-intron";#new: added "-intron" at the end of id
        		        }
        		    }
        		}
        		if ($printed) {
	...
	print STDERR "Processed $site_count SNPs\n";#new: added stderr message
}
...
sub _load_site_filter() {
    ...
    if (defined($included_sites_vcf_file)) {
        ...
        while (my $line = <SITES>) {
            ...
            my $chr = $ts[0];
        	$chr =~ s/chr//;
            $site_info{$chr}{$ts[1]}{'id'} = $ts[2];
            $site_info{$chr}{$ts[1]}{'alleles'} = $ts[3] . "/" . $ts[4];
        }
        close(SITES);
        return \%site_info;
    } else {
