...
KALLISTO_INDEX=/data/srlab/external-data/Ensembl/pub/release-81/fasta/homo_sapiens/cdna/kallisto/Homo_sapiens.GRCh38.cdna.all
...
for path in ${paths[*]}; do
  ...
  job="kallisto_ensembl81_$(date +%Y-%m-%d_%H-%M)"
  ...
  ls $path/*/*.fastq.gz | sort | while read fq1; read fq2; do
	  ...
	  out=$path/kallisto_ensembl81/$sample1
	  mkdir -p $out
