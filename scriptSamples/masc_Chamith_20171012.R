# Notes added by Christin
# From Chamith: It'll run best if you organize the data into a dataframe, then give it the names of the appropriate columns.  Needs single cell data, a vector of cluster assignments, and covariates (if any).
# Helpful website: http://blog.minitab.com/blog/adventures-in-statistics-2/understanding-analysis-of-variance-anova-and-the-f-test
...
MASC <- function(data, cluster, contrast, random.effects, fixed.effects, verbose = TRUE) {
  ...
  for (i in 1:nlevels(cluster)) {
    ...
    null.model <- as.formula(paste(paste("cluster", i, " ~ 1", sep = ""), model.fixeff, model.randeff, sep = " + "))
    full.model <- as.formula(paste(paste("cluster", i, " ~ ", contrast, sep = ""), model.fixeff, model.randeff, sep = " + "))
